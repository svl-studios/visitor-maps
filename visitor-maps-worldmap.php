...
if ( isset ( $MS['time'] ) && is_numeric( $MS['time'] ) && isset ( $MS['units'] ) ) {
	...
	if ( $time > 0 && ( $units == 'minute' || $units == 'minutes' ) ) {
		...
		$G['time']      = $time;
		$G['units']     = $units;
	} elseif ( $time > 0 && ( $units == 'hour' || $units == 'hours' ) ) {
		$seconds_ago    = ( $time * 60 * 60 ); // hours
		$units_filtered = $units;
		$G['time']      = $time;
		$G['units']     = $units;
	} elseif ( $time > 0 && ( $units == 'day' || $units == 'days' ) ) {
...
if ( isset ( $MS['type'] ) && $MS['type'] == 'jpg' ) {
	$type = 'jpg';
} elseif ( isset ( $MS['type'] ) && $MS['type'] == 'png' ) {
	$type = 'png';
} else {
	$type = 'png';
}
...
if ( ! Visitor_Maps::$core->get_option( 'hide_bots' ) ) {
	...
	$rows_arr = $wpdb->get_results( "
                 ...
                 FROM " . $wo_table_wo . "
                 WHERE time_last_click > '" . absint( $xx_secs_ago ) . "' LIMIT " . absint( Visitor_Maps::$core->get_option( 'pins_limit' ) ) . "", ARRAY_A );
	...
	$rows_arr = $wpdb->get_results( "
                 ...
                 OR (name != 'Guest' AND user_id > 0 AND time_last_click > '" . absint( $xx_secs_ago ) . "') LIMIT " . absint( Visitor_Maps::$core->get_option( 'pins_limit' ) ) . "", ARRAY_A );
...
if ( $rows_arr ) { // check of there are any visitors
	...
	foreach ( $rows_arr as $row ) {
		if ( $row['longitude'] != '0.0000' && $row['latitude'] != '0.0000' ) {
			...
			if ( ! Visitor_Maps::$core->get_option( 'hide_bots' ) && $row['user_id'] == 0 && $row['name'] != 'Guest' ) {
				//  find name for bot
...
$maps_in_tables and $string .= '</tr>';
$maps_in_tables and $string .= '</table>';