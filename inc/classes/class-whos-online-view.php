...
if ( ! class_exists( 'Whos_Online_View' ) ) {
	...
	class Whos_Online_View {
		 ...
		 */
		public function view_whos_online(): void {
			global $wpdb;
				...
				<tr>
					<td class="actions">
						<form name="wo_view" action="<?php echo esc_url( admin_url( 'admin.php?page=visitor-maps' ) ); ?>" method="get">
							...
							<?php wp_nonce_field( 'wo_view', 'wo_view_nonce' ); ?>
							...
							if ( $this->set['allow_profile_display'] ) {
								...
								echo '<div class="profile select-wrapper"><span class="label">' . esc_html__( 'Profile Display:', 'visitor-maps' ) . '</span> ' . $this->draw_pull_down_menu( 'show', $show_type, $show, 'onchange="this.form.submit();"' ) . '</div> ';
							}

							if ( $this->set['allow_refresh'] ) {
								// phpcs:ignore WordPress.Security.EscapeOutput
								echo '<div class="refresh select-wrapper"><span class="label">' . esc_html__( 'Refresh Rate:', 'visitor-maps' ) . '</span> ' . $this->draw_pull_down_menu( 'refresh', $refresh_values, $refresh, 'onchange="this.form.submit();"' ) . '</div> ';
							}

							// phpcs:ignore WordPress.Security.EscapeOutput
							echo '<div class="show-bots select-wrapper"><span class="label">' . esc_html__( 'Show Bots:', 'visitor-maps' ) . '</span> ' . $this->draw_pull_down_menu( 'bots', $bots_type, $bots, 'onchange="this.form.submit();"' ) . '</div> ';
							?>
							<input type="hidden" name="page" value="visitor-maps"/>
						</form>
						<br /><br /><br /><a href="<?php echo esc_url( admin_url( 'admin.php?page=whos-been-online' ) ); ?>"><?php echo esc_html__( "Who's Been Online", 'visitor-maps' ); ?></a> |
						<?php
						...
						if ( Visitor_Maps::$core->get_option( 'enable_location_plugin', true ) ) {
							echo ' | <a class="map-console-bottom" href="' . esc_url( get_bloginfo( 'url' ) ) . '?wo_map_console=1">' . esc_html__( 'Visitor Map Viewer', 'visitor-maps' ) . '</a>';
						}
						?>
					</td>
					<td>
											...
											<tr>
												<td colspan="9"><br/>
													<table class="visitor-stats">
														<tr>
															<td style="text-align:right;padding-right:0;"><strong><?php echo esc_html( "$total_sess" ); ?></strong></td>
															<?php // translators: %1$d = Inactive minutes, %2$d = Removed minutes. ?>
															<td style="text-align:left;"><?php printf( esc_html__( 'Visitors online (Considered inactive after %1$d minutes. Removed after %2$d minutes)', 'visitor-maps' ), absint( Visitor_Maps::$core->get_option( 'active_time' ) ), absint( Visitor_Maps::$core->get_option( 'track_time' ) ) ); ?></td>
														</tr>
														<?php
														if ( $total_dupes > 0 ) {
															?>
															<tr>
																<td style="text-align:right;padding-right:0;"><strong><?php echo esc_html( "$total_dupes" ); ?></strong></td>
														...
														?>
														<tr>
															<td style="text-align:right;padding-right:0;"><strong><?php echo esc_html( "$total_users" ); ?></strong></td>
															<td><?php echo esc_html__( 'Members (includes you)', 'visitor-maps' ); ?></td>
														</tr>
														<tr>
															<td style="text-align:right;padding-right:0;"><strong><?php echo esc_html( "$total_guests" ); ?></strong></td>
														...
														<tr>
															<td style="text-align:right;padding-right:0;"><strong><?php echo esc_html( "$total_bots" ); ?></strong></td>
															<td><?php echo esc_html__( 'Bots', 'visitor-maps' ); ?></td>
														</tr>
														<tr>
															<td style="text-align:right;padding-right:0;"><strong><?php echo esc_html( "$total_admin" ); ?></strong></td>
		...
		private function draw_pull_down_menu( string $name, array $values, string $def = '', string $parameters = '' ): string {
			...
			if ( empty( $def ) && isset( $_GET['wo_view_nonce'] ) && wp_verify_nonce( sanitize_key( $_GET['wo_view_nonce'] ), 'wo_view' ) && ( ( isset( $_GET[ $name ] ) && is_string( $_GET[ $name ] ) ) || ( isset( $_POST[ $name ] ) && is_string( $_POST[ $name ] ) ) ) ) {
				if ( isset( $_GET[ $name ] ) && is_string( $_GET[ $name ] ) ) {
					$def = sanitize_text_field( wp_unslash( $_GET[ $name ] ) );
