...
if ( ! class_exists( 'Whos_Online_Been' ) ) {
	...
	class Whos_Online_Been {
		...
		public function view_whos_been_online() {
			...
			echo '<tr><td>';
			echo '<form name="wo_been" action="' . esc_url( admin_url( 'index.php?page=whos-been-online' ) ) . '" method="get">';
			...
			echo '</form>';
			echo '<a href="' . esc_url( admin_url( 'index.php?page=visitor-maps' ) ) . '">' . esc_html__( 'Who\'s Online', 'visitor-maps' ) . "</a>\n";
			...
			<table border="0" cellspacing="2" cellpadding="2" width="99%">
				<tr>
					<td align="center">
						...
						<b><?php echo sprintf( esc_html__( '%1$d visitors since %2$s', 'visitor-maps' ), (int) $numrows, ( intval( $numrows ) > 0 ) ? gmdate( Visitor_Maps::$core->get_option( 'date_time_format' ), (int) $since ) : __( 'installation', 'visitor-maps' ) ); ?></b>
					</td>
				</tr>
				<tr>
					<td align="left">
						...
						if ( 1 === $pageno ) {
							...
							// phpcs:ignore WordPress.Security.EscapeOutput
							echo ' <a href=' . esc_url( admin_url( 'index.php?page=whos-been-online' ) ) . "&amp;pageno=1$getstring>&laquo;" . esc_html__( 'FIRST', 'visitor-maps' ) . '</a> ';
							$prevpage = $pageno - 1;

							// phpcs:ignore WordPress.Security.EscapeOutput
							echo ' <a href=' . esc_url( admin_url( 'index.php?page=whos-been-online' ) ) . "&amp;pageno=$prevpage$getstring>&lsaquo;" . esc_html__( 'PREV', 'visitor-maps' ) . '</a> ';
						}
						...
						if ( $pageno === $lastpage ) {
							...
							// phpcs:ignore WordPress.Security.EscapeOutput
							echo ' <a href=' . esc_url( admin_url( 'index.php?page=whos-been-online' ) ) . "&amp;pageno=$nextpage$getstring>" . esc_html__( 'NEXT', 'visitor-maps' ) . '&rsaquo;</a> ';

							// phpcs:ignore WordPress.Security.EscapeOutput
							echo ' <a href=' . esc_url( admin_url( 'index.php?page=whos-been-online' ) ) . "&amp;pageno=$lastpage$getstring>" . esc_html__( 'LAST', 'visitor-maps' ) . '&raquo;</a> ';
						}
						?>
					</td>
				</tr>
										...
										if ( $whos_online_arr ) { // check of there are any visitors.
											foreach ( $whos_online_arr as $whos_online ) {
												...
												if ( ! ( $is_bot && ! $bots ) ) {
													...
													<!-- Name -->
													<?php
													echo '<td valign="top">&nbsp;<font color="' . esc_attr( $fg_color ) . '">';
													...
													if ( $is_guest ) {
														...
														echo '<a href="' . esc_url( admin_url( 'user-edit.php?user_id=' . (int) $whos_online['user_id'] ) ) . '">' . esc_html( __( 'You', 'visitor-maps' ) ) . '</a>&nbsp;';
														// Check for Bot.
													} elseif ( $is_bot ) {
													...
													if ( $this->set['allow_ip_display'] ) {
														...
														<!-- Visits -->
														<td valign="top">&nbsp;<font
																	color="<?php echo esc_attr( $fg_color ); ?>"><?php echo esc_html( $whos_online['num_visits'] ); ?></font>
														</td>

														<!-- Last Visit -->
														<td valign="top">&nbsp;<font
																	color="<?php echo esc_attr( $fg_color ); ?>"><?php echo esc_html( gmdate( Visitor_Maps::$core->get_option( 'date_time_format' ), $whos_online['time_last_click'] ) ); ?></font>
														</td>
