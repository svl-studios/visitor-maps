...
if ( ! class_exists( 'WoBeen' ) ) {
	class WoBeen {
		...
		function view_whos_been_online() {
			...
			echo '<tr><td>';
			echo '<form name="wo_been" action="' . admin_url( 'index.php?page=whos-been-online' ) . '" method="get">';

			if ( $this->set['allow_profile_display'] ) {
				echo esc_html( __( 'Profile Display:', 'visitor-maps' ) ) . ' ' . $this->draw_pull_down_menu( 'show', $show_arr, $show, 'onchange="this.form.submit();"' ) . ' ';
			}
			...
			echo '</form>';
			echo '<a href="' . admin_url( 'index.php?page=visitor-maps' ) . '">' . esc_html( __( 'Who\'s Online', 'visitor-maps' ) ) . "</a>\n";

			if ( current_user_can( 'manage_options' ) ) {
				echo '<br /> <a href="' . admin_url( 'options-general.php?page=visitor_maps_opt' ) . '">' . __( 'Visitor Maps Options', 'visitor-maps' ) . "</a>\n";
			}
			...
			<td>
				<table border="0" cellspacing="2" cellpadding="2" align="right">
					<tr>
						<td><?php echo '<img src="' . Visitor_Maps::$url . 'img/maps/' . $this->set['image_active_guest'] . '" border="0" alt="' . esc_attr( __( 'Active Guest', 'visitor-maps' ) ) . '" title="' . esc_attr( __( 'Active Guest', 'visitor-maps' ) ) . '" /> ' . esc_html( __( 'Active Guest', 'visitor-maps' ) ); ?>
						</td>
						<td><?php echo '<img src="' . Visitor_Maps::$url . 'img/maps/' . $this->set['image_inactive_guest'] . '" border="0" alt="' . esc_attr( __( 'Inactive Guest', 'visitor-maps' ) ) . '" title="' . esc_attr( __( 'Inactive Guest', 'visitor-maps' ) ) . '" /> ' . esc_html( __( 'Inactive Guest', 'visitor-maps' ) ); ?>
						</td>
					</tr>
					<tr>
						<td><?php echo '<img src="' . Visitor_Maps::$url . 'img/maps/' . $this->set['image_active_bot'] . '" border="0" alt="' . esc_attr( __( 'Active Bot', 'visitor-maps' ) ) . '" title="' . esc_attr( __( 'Active Bot', 'visitor-maps' ) ) . '" /> ' . esc_html( __( 'Active Bot', 'visitor-maps' ) ); ?>
						</td>
						<td><?php echo '<img src="' . Visitor_Maps::$url . 'img/maps/' . $this->set['image_inactive_bot'] . '" border="0" alt="' . esc_attr( __( 'Inactive Bot', 'visitor-maps' ) ) . '" title="' . esc_attr( __( 'Inactive Bot', 'visitor-maps' ) ) . '" /> ' . esc_html( __( 'Inactive Bot', 'visitor-maps' ) ); ?>
						</td>
					</tr>
				</table>
			</td>
			...
			<table border="0" cellspacing="2" cellpadding="2" width="99%">
				<tr>
					<td align="center">
						<b><?php echo sprintf( __( '%1$d visitors since %2$s', 'visitor-maps' ), (int) $numrows, ( $numrows > 0 ) ? date( Visitor_Maps::$core->get_option( 'date_time_format' ), (int) $since ) : __( 'installation', 'visitor-maps' ) ); ?></b>
					</td>
				</tr>
				<tr>
					<td align="left">
								...
								<td valign="top">
									<table border="0" cellspacing="0" cellpadding="2" width="99%">
										<tr class="table-top">
											...
											<?php if ( $this->set['allow_ip_display'] ) {
												echo '<td>&nbsp;' . esc_html( __( 'IP Address', 'visitor-maps' ) ) . '</td> ';
											} ?>
											<?php if ( Visitor_Maps::$core->get_option( 'enable_location_plugin' ) ) {
												echo '<td>&nbsp;' . esc_html( __( 'Location', 'visitor-maps' ) ) . '</td> ';
											} ?>
											...
											?>
											<?php if ( $this->set['allow_referer_display'] ) {
												echo '<td>&nbsp;' . esc_html( __( 'Referer', 'visitor-maps' ) ) . '</td> ';
											} ?>
										</tr>
										...
										if ( $whos_online_arr ) { // check of there are any visitors
											foreach ( $whos_online_arr as $whos_online ) {
												...
												if ( $whos_online['name'] != 'Guest' && $whos_online['user_id'] == 0 ) {
													...
													$is_bot   = true;
												} elseif ( $whos_online['name'] != 'Guest' && $whos_online['user_id'] > 0 && $whos_online['ip_address'] != $this->wo_visitor_ip ) {
													$total_users ++;
												...
												if ( ! ( $is_bot && ! $bots ) ) {
													...
													<!-- Status Light -->
													<td align="left"
													    valign="top"><?php echo $this->check_status( $whos_online ); ?></td>

													<!-- Name -->
													<?php
													echo '<td valign="top">&nbsp;<font color="' . $fg_color . '">';
													...
													if ( $this->set['allow_ip_display'] ) {
														...
														<!-- Visits -->
														<td valign="top">&nbsp;<font
																	color="<?php echo $fg_color; ?>"><?php echo esc_html( $whos_online['num_visits'] ) ?></font>
														</td>

														<!-- Last Visit -->
														<td valign="top">&nbsp;<font
																	color="<?php echo $fg_color; ?>"><?php echo date( Visitor_Maps::$core->get_option( 'date_time_format' ), $whos_online['time_last_click'] ); ?></font>
														</td>
													...
													if ( Visitor_Maps::$core->get_option( 'enable_location_plugin' ) ) {
														...
														if ( $whos_online['country_code'] != '' ) {
															...
															if ( $country_code == '-' || $country_code == '--' ) { // unknown
																echo '<img src="' . Visitor_Maps::$url . 'img/flags/unknown.png" alt="' . esc_attr( __( 'unknown', 'visitor-maps' ) ) . '" title="' . esc_attr( __( 'unknown', 'visitor-maps' ) ) . '" />';
															} else {
																echo '<img src="' . Visitor_Maps::$url . 'img/flags/' . $country_code . '.png" alt="' . esc_attr( $whos_online['country_name'] ) . '" title="' . esc_attr( $whos_online['country_name'] ) . '" />';
															}
														}
													...
													if ( ( $this->set['allow_last_url_display'] ) && ( ( isset ( $_GET['nlurl'] ) ) || ( $this->set['allow_profile_display'] && $show != 'none' ) ) ) {
														...
														<td style="text-align:left"
														    colspan="8"><?php echo esc_html( __( 'Last URL:', 'visitor-maps' ) ) . ' <a href="' . esc_url( $whos_online['last_page_url'] ) . '" target="_blank">' . esc_html( $display_link ) . '</a>'; ?></td>
														</tr>
														<?php
													}
		...
		function check_status( $whos_online ) {
			...
			if ( $whos_online['name'] != 'Guest' && $whos_online['user_id'] == 0 ) {   // bot
				if ( $whos_online['time_last_click'] < $xx_mins_ago_long ) {
					return '<img src="' . Visitor_Maps::$url . 'img/maps/' . $this->set['image_inactive_bot'] . '" border="0" alt="' . esc_attr( __( 'Inactive Bot', 'visitor-maps' ) ) . '" title="' . esc_attr( __( 'Inactive Bot', 'visitor-maps' ) ) . '" />';
				} else {
					return '<img src="' . Visitor_Maps::$url . 'img/maps/' . $this->set['image_active_bot'] . '" border="0" alt="' . esc_attr( __( 'Active Bot', 'visitor-maps' ) ) . '" title="' . esc_attr( __( 'Active Bot', 'visitor-maps' ) ) . '" />';
				}
			} else {  // guest
				if ( $whos_online['time_last_click'] < $xx_mins_ago_long ) {
					return '<img src="' . Visitor_Maps::$url . 'img/maps/' . $this->set['image_inactive_guest'] . '" border="0" alt="' . esc_attr( __( 'Inactive Guest', 'visitor-maps' ) ) . '" title="' . esc_attr( __( 'Inactive Guest', 'visitor-maps' ) ) . '" />';
				} else {
					...
					return '<img src="' . Visitor_Maps::$url . 'img/maps/' . $this->set['image_active_guest'] . '" border="0" alt="' . esc_attr( __( 'Active Guest', 'visitor-maps' ) ) . '" title="' . esc_attr( __( 'Active Guest', 'visitor-maps' ) ) . '" />';
				}
			}
		}
		...
		function display_details( $whos_online ) {
			...
			if ( Visitor_Maps::$core->get_option( 'enable_host_lookups' ) ) {
				$this_host = ( $whos_online['hostname'] != '' ) ? Visitor_Maps_Core::host_to_domain( $whos_online['hostname'] ) : 'n/a';
